...
EMOJI_COPY_BTN    = discord.PartialEmoji(name="copy_text", id=1526743644894138479)
EMOJI_DISCORD_BTN = discord.PartialEmoji(name="discord", id=1526743527642501273)
EMOJI_INVITE_BTN  = discord.PartialEmoji(name="voice_invite", id=1526743390488756236)
...
# ── HELPERS ──────────────────────────────────────────────────────
BOT_START_TIME = dt.now(timezone.utc)
...
def _uptime() -> str:
    d = dt.now(timezone.utc) - BOT_START_TIME
...
# ── EMBEDS DE BYPASS ──────────────────────────────────────────
def embed_loading():
    e = discord.Embed(color=C_WARN, timestamp=dt.now(timezone.utc))
...
def embed_success(result: str, elapsed: float, platform_emoji: str):
    e = discord.Embed(color=C_GREEN, timestamp=dt.now(timezone.utc))
...
def embed_fail(error: str, elapsed: float, platform_emoji: str):
    e = discord.Embed(color=C_ERROR, timestamp=dt.now(timezone.utc))
    e.title = f"{EMOJI_RED_DOT} Bypass Failed"
    ...
    while seconds > 0:
        try:
            ...
            for i, field in enumerate(new_embed.fields):
                if field.name == f"{EMOJI_CLOCK} Auto Delete":
                    new_embed.set_field_at(i, name=field.name, value=f"Message expires in: `{seconds}s`", inline=field.inline)
                    field_updated = True
                    break
            if not field_updated:
                new_embed.add_field(name=f"{EMOJI_CLOCK} Auto Delete", value=f"Message expires in: `{seconds}s`", inline=False)
...
async def gv_crear(interaction: discord.Interaction, premio: str, minutos: int, ganadores: int = 1):
    end_time = dt.now(timezone.utc) + datetime.timedelta(minutes=minutos)
    e = discord.Embed(
        title=f"{EMOJI_GIVEAWAY} GIVEAWAY",
...
async def _giveaway_countdown(msg: discord.Message, premio: str, ganadores: int, end_time: dt, channel: discord.TextChannel):
    await asyncio.sleep(max(0, (end_time - dt.now(timezone.utc)).total_seconds()))
    ...
    e = discord.Embed(
        title=f"{EMOJI_GOLDEN_SPIN_COIN} ¡GIVEAWAY TERMINADO!",
        description=f"**Premio:** {premio}\n🏆 **Ganador(es):** {mentions}",
        color=C_GREEN
    )
    e.set_thumbnail(url="https://cdn.discordapp.com/emojis/1526742765311098980.gif")
...
async def gv_terminar(interaction: discord.Interaction, message_id: str):
    try:
        ...
        e = discord.Embed(
            title=f"{EMOJI_GOLDEN_SPIN_COIN} Giveaway Terminado",
            description=f"**Premio:** {data['prize']}\n🏆 **Ganador(es):** {mentions}",
            color=C_GREEN
        )
        e.set_thumbnail(url="https://cdn.discordapp.com/emojis/1526742765311098980.gif")
        e.set_footer(text=_footer())
...
async def gv_reroll(interaction: discord.Interaction, message_id: str):
    try:
        ...
        e = discord.Embed(
            title=f"{EMOJI_DIAMOND} Nuevo Ganador",
            description=f"🏆 ¡{winner.mention} ganó el re-roll!",
            color=C_GREEN
        )
        e.set_footer(text=_footer())
...
async def cmd_help(interaction: discord.Interaction):
    e = discord.Embed(
        title=f"{EMOJI_INFORMATION} Centro de Ayuda",
        description="Comandos disponibles. Usa `/comando` para ejecutar.",
        color=C_GREEN
    )
    e.set_thumbnail(url="https://cdn.discordapp.com/emojis/1526742765311098980.gif")
...
async def cmd_mute(interaction: discord.Interaction, usuario: discord.Member, minutos: int = 10):
    await usuario.timeout(dt.now(timezone.utc) + datetime.timedelta(minutes=minutos), reason="Mute aplicado")
...
async def cmd_gif(interaction: discord.Interaction, busqueda: str):
    await interaction.response.defer()
    r = requests.get(f"https://tenor.googleapis.com/v2/search?q={busqueda}&key=AIzaSyC2-7bLmQ0lB7p3mO_qB3X0D5TdYbKjU8s&limit=1&media_filter=gif").json()
    if not r['results']:
        return await interaction.followup.send("❌ No se encontraron GIFs.")
    await interaction.followup.send(r['results'][0]['media_formats']['gif']['url'])
...
def _eco_get(uid):
    ...
    if uid not in economy_data:
        economy_data[uid] = {"bal": 100, "daily": 0, "inv": []}
...
async def cmd_shop(interaction: discord.Interaction):
    e = discord.Embed(title=f"{EMOJI_CART} Tienda", color=C_GREEN)
    e.add_field(name="Item 1", value="50 monedas", inline=True)
    e.add_field(name="Item 2", value="100 monedas", inline=True)
    e.set_footer(text=_footer())
    await interaction.response.send_message(embed=e)

@bot.tree.command(name="buy", description="🛒 Comprar un item")
@app_commands.describe(item="Nombre del item")
async def cmd_buy(interaction: discord.Interaction, item: str):
    await interaction.response.send_message("⏳ Sistema de compras en desarrollo.", ephemeral=True)
...
async def cmd_rolelist(interaction: discord.Interaction):
    roles = interaction.guild.roles[1:]  # skip @everyone
    if not roles:
        return await interaction.response.send_message("No hay roles.", ephemeral=True)
        ...
        if urls:
            for url in urls[:3]:
                if _is_valid_url(url):
                    result, error = await asyncio.get_running_loop().run_in_executor(None, _bypass_sync, url)
                    if result:
                        e = discord.Embed(color=C_GREEN, description=f"{EMOJI_GOLD_KEY2} **Result:**\n```txt\n{result[:900]}\n```")
                        await message.channel.send(content=message.author.mention, embed=e)
    # XP system
